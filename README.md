...
2. Modify middleware methods to handle multiple image file uploads, max 5. If more than 5 images, it'll ignore the excess and handle the first 5. Took the approach of using an array to store the images rather than another Model for images. 

3. Generate new migration files to reflect changes in dataType.

4. Sort posts by newest with time difference calculated on get all posts route. Utilized a library and native get method.

5. Store only the unique part of the s3 links in postgres. For optimization, removed the domain at storage.

6. Handle edit requests for title and description

7. Handle edit requests for new file uploads and keep 5 max. Checks in place to limit 5. It will throw an error if user tries to add more images.
